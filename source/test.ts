...
import supportsHyperlinks from './index.js';

const isSupported = (
	{platform, env, argv, stream}:
	{platform?: string; env?: NodeJS.ProcessEnv; argv?: string[]; stream?: WriteStream},
) => {
	...
	const result = supportsHyperlinks(stream);
...
test('not supported terminal emulator', t => {
	t.false(isSupported(
		{
			env: {
				TERM_PROGRAM: 'UnicornTerm',
				TERM_PROGRAM_VERSION: 'INVALID_VERSION',
			},
			// @ts-expect-error: Mock WriteStream
			stream: {
				isTTY: true,
			},
		},
...
test('not supported vscode 1.71.0', t => {
	t.false(isSupported({
		env: {
			TERM_PROGRAM: 'vscode',
...
test('supported vscode 1.72.0', t => {
	t.true(isSupported({
		env: {
			TERM_PROGRAM: 'vscode',
